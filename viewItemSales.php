...
<?php
		...
		$query = "SELECT itemImage, description, unitCost, quantityTook, personWhoTook, personReceiving, type, date FROM Items, Sales WHERE Items.ID = Sales.ID ORDER BY date";
		$result = mysqli_query($con, $query);
	
		echo "<h1>Transaction History | Items</h1>";
		...
		while($row=mysqli_fetch_array($result)) {
			echo "<tr>";
				...
				echo "<td align='center' width='15%'>{$row['quantityTook']}</td>";
				echo "<td align='center' width='15%'>{$row['personWhoTook']}</td>";
