...
<head>
<meta charset="utf-8">
<title>Inactive Printed Material | UNI CBB</title>
...
<?php
    	...
    	$query = "SELECT * FROM `Marketing_Material` WHERE `isActive` = 'Inactive'";
		$result = mysqli_query($con, $query);

			echo "<h1>Current Inventory | Inactive Printed Material</h1>";
			...
			<?php
			echo "<a href='viewMarketingMaterials.php'><button class='button'>Back</button></a>";
		...
		while($row=mysqli_fetch_array($result)) {
			echo "<tr>";
				...
				echo "<td align='center' width='80px'>{$row['isActive']}<br><a href='editMaterialsStatus.php?update={$row['ID']}'><img src='/InventoryManager/InventoryManagerImages/Icons/pencil-edit-button.png' title='Edit the status of the selected row'><br><a href='deleteApparel.php?ID={$row['ID']}'><img src='/InventoryManager/InventoryManagerImages/Icons/cross.png' title='Delete the selected row from inventory'></td>";
				echo "<td align='center' width='110px'><img src='InventoryManagerImages/MktgMaterials/{$row['materialsImage']}' width='127' height='125' style='display:block'></td>";
			echo "</tr>";
		}
		echo "</table>";
		echo "</form>";
